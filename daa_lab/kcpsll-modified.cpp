...
ND* bf_bb_knapsack(itm* item, float wl, int size){
	...
	while(clvl<=size && !q.emptyList()){
		...
		//cout<<"So far so good!\n";
		lchild->wu=nd->wu+item[clvl-1].w;
		//cout<<nd->wu+item[clvl].w<<endl;
		//cout<<"So far so good!\n";
		if(lchild->wu<=wl){
			lchild->vg=nd->vg+item[clvl-1].v;
			lchild->parent=nd;
			lchild->lvl=nd->lvl+1;
			if(clvl==size)
				lchild->ub=lchild->vg;
			else
				lchild->ub=lchild->vg+(wl-lchild->wu)*item[clvl-1].vbw;
		...
		if(clvl==size)
			...
			rchild->ub=rchild->vg+(wl-rchild->wu)*item[clvl-1].vbw;
